...
from .util import make_dirs, bbox_offset
...
class FrameReader(Reader):
    ...
    def _save_file(self, output_dir, i, suffix, file_):
        if len(self.filenames) == len(self.files):
            name = sorted(self.filenames)[i].split('/')[-1]
        else:
            name = f"frame_{i:04}_{suffix}.png"
...
def save_frames_to_dir(frames, start_idx, suffix, dirname):
    reader = FrameReader(dirname, read=False)
    reader.set_files(frames)
    reader.save_files(start_idx, suffix, dirname)
